	...
	open: function (id) {
		...
		targetLayer.addEventListener('mouseleave', function () {
			...
			if (id == 'layerPromoList') {
				slide.obj['layerPromoList'].destroy();
			}
		});
	...
	create: function (vodId, tagId, type, auto, thumb, href, sticky) {
		...
		if (type == 'vimeo') {
			...
			try {
				...
				setTimeout(function () {
					vodPlayer.create(vodId, tagId, 'vimeo', auto);
				}, 100);
			}
		}
...
var methodShow = function (state) {
	...
	(state == 'show') ? methodCont.classList.add('open'): methodCont.classList.remove('open');
}
