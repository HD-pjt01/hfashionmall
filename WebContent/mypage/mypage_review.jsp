...
<body>
	<script type="text/javascript">
		window.onload = function() {
			const reviewPopup = document.getElementsByClassName('reviewPopup');
			const reviewOpenBtn = document
					.getElementsByClassName('btn-type3-sm');
			const reviewCloseBtn = document
					.getElementsByClassName('reviewClose');

			for (let i = 0; i < reviewPopup.length; i++) {
				reviewOpenBtn[i].addEventListener('click', function() {
					reviewPopup[i].style.display = "block";
				});

				reviewCloseBtn[i].addEventListener('click', function() {
					reviewPopup[i].style.display = "none";
				});
			}
		}
	</script>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${orderList}" var="orderVO">
									...
									<div class="reviewPopup" style="display: none;">
                            ...
                            <div class="point-select">
                                <span>
                                    ...
                                    <label for="pdPointSel1">1점</label>
													...
													<button type="button" id="reviewClose" class="reviewClose btn-layer-close">닫기</button>
												</div>
											</div>
										</form>
									</div>
